...
function layerDataReset(layer, keep = []) {
	...
	if(layer == "u") {
		if(player.s.unlock && !hasUpgrade("qol", 41)) player[layer].upgrades.push(11)
	}
...
function gameLoop(diff) {
	...
	if(hasUpgrade("qol", 61)) {
		...
		if(player.u.buyables[12].gte(100)) buyBuyable("u", 12)
		if(player.u.buyables[13].gte(100)) buyBuyable("u", 13)
		if(player.h.buyables[11].gte(100)) buyBuyable("h", 11)
		if(player.h.buyables[12].gte(100)) buyBuyable("h", 12)
	}
...
var interval = setInterval(function() {
	...
	if (player.offTime !== undefined) {
		...
		if (player.offTime.remain > 0) {
			let offlineDiff = Math.max(player.offTime.remain / 100, diff)
